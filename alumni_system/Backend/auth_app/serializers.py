...
from .models import CustomUser, Skill, WorkHistory, AlumniDirectory, SurveyCategory, SurveyQuestion, SurveyResponse
import re
...
class SurveyCategorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'name', 'order', 'is_active', 'created_at', 'questions_count']
...
class SurveyQuestionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'options', 'is_required', 'order', 'is_active', 'created_at', 'updated_at'
        ]
...
class SurveyResponseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'response_text', 'response_number', 'response_date', 'response_json'
        ]

class SurveyResponseCreateSerializer(serializers.Serializer):
    """Serializer for creating multiple survey responses during registration"""
    ...
    def validate_responses(self, value):
        ...
        question_ids = [response.get('question_id') for response in value]
        required_questions = SurveyQuestion.objects.filter(
            is_active=True, 
            is_required=True
        ).values_list('id', flat=True)
        
        missing_questions = set(required_questions) - set(question_ids)
        if missing_questions:
            missing_question_texts = SurveyQuestion.objects.filter(
                id__in=missing_questions
            ).values_list('question_text', flat=True)
            raise serializers.ValidationError(
                f"Missing responses for required questions: {', '.join(missing_question_texts)}"
            )
