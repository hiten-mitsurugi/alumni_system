...
const handlePostCreated = (newPost) => {
  posts.value.push({
    ...
    category: newPost.content_category.charAt(0).toUpperCase() + newPost.content_category.slice(1),
  });
};

onMounted(() => {
  websocketService.connect('notifications');
  websocketService.addListener('notifications', (data) => {
    console.log('Notification:', data);
  });

  if (route.params.id) {
    websocketService.connect(`post/${route.params.id}`);
    websocketService.addListener(`post/${route.params.id}`, (data) => {
...
onUnmounted(() => {
  ...
  if (route.params.id) {
    websocketService.disconnect(`post/${route.params.id}`);
  }
});
</script>
...
<template>
  <div class="p-6">
    ...
    <!-- Modal -->
    <CreatePostModal v-if="isModalOpen" :is-open="isModalOpen" @close="isModalOpen = false" @post-created="handlePostCreated" />
  </div>
</template>