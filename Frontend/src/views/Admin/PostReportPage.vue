...
const confirmAction = async () => {
  try {
    console.log('🎯 Confirming action:', actionModal.value.action, 'for report:', actionModal.value.report.id)

    const response = await axios.post(`${BASE_URL}/api/posts/reports/${actionModal.value.report.id}/action/`, {
      action: actionModal.value.action,
      note: actionModal.value.note
    }, {
      headers: { Authorization: `Bearer ${authStore.token}` }
    })
    ...
    // Remove from reports list
    reportedPosts.value = reportedPosts.value.filter(report => report.id !== actionModal.value.report.id)
    ...
    successMessage.value = actionMessages[actionModal.value.action] || response.data.message
    ...
    console.error(`Error ${actionModal.value.action}ing report:`, error)
    errorMessage.value = `Failed to ${actionModal.value.action} report. Please try again.`
    setTimeout(() => {
      errorMessage.value = ''
    }, 5000)
  }
...
const getReasonColor = (reason) => {
  const colors = {
    ...
    'fake_news': 'bg-purple-100 text-purple-800',
