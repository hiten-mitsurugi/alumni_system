<template>
      ...
      <div class="w-80 border-r border-gray-200 flex-shrink-0">
        ...
        <div class="overflow-y-auto">
          <div 
            ...
            :class="[
              'flex items-center p-4 cursor-pointer border-b border-gray-100 hover:bg-gray-50',
              selectedConversation?.id === conversation.id ? 'bg-blue-50 border-r-4 border-blue-500' : ''
            ]"
          >
            <img 
              :src="getProfilePictureUrl(conversation.mate || conversation.group)" 
          ...
          <div class="p-4 bg-white border-b border-gray-200">
            <h3 class="font-semibold text-gray-900">
              {{ selectedConversation.type === 'private' 
                  ? `${selectedConversation.mate.first_name} ${selectedConversation.mate.last_name}` 
 ...
 <div :class="[
 'border-l border-gray-200/60 w-80 lg:w-96',
 // Simple visibility based on showChatInfo state
 { 
 'block': showChatInfo && (!isMobile || currentMobileView === 'chat-info'),
 'hidden': !showChatInfo || (isMobile && currentMobileView !== 'chat-info')
 }
 ]">
 <ChatInfoPanel 
...
<style scoped>
/* Minimal styles to avoid interfering with AlumniLayout */
.messaging-container {
...
/* Basic transition for smooth show/hide */
.block,
.hidden {
...
// Initialize mobile state immediately to prevent layout jumping
if (typeof window !== 'undefined') {
  isMobile.value = window.innerWidth < 768;
  console.log('📱 Initial mobile state set:', isMobile.value);
}

// Watch for route changes and handle them gracefully (after state initialization)
watch(
  () => route.fullPath,
  (newPath, oldPath) => {
    if (newPath === '/alumni/messaging' && oldPath !== '/alumni/messaging') {
      console.log('📱 Route changed to messaging, resetting mobile view to prevent layout issues');
      // Reset mobile view to list when navigating to messaging
      currentMobileView.value = 'list';
      showChatInfo.value = false;
    }
  }
  // Remove immediate: true to prevent initialization issues
);
...
function checkScreenSize() {
 ...
 // Reset mobile view to list when switching to desktop
 if (!newIsMobile) {
 currentMobileView.value = 'list';
 }
 }
