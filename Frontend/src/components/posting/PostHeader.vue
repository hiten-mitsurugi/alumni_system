    ...
    <div class="flex items-start space-x-4">
      ...
      <div>
        <div class="flex items-center space-x-3">
          ...
          <span v-if="post.user?.user_type <= 2" class="px-3 py-1 bg-gradient-to-r from-blue-500 to-blue-600 text-white text-sm font-bold rounded-full shadow-md">
        ...
        <div class="flex items-center space-x-3 text-lg text-slate-600 mt-1">
          ...
          <span class="capitalize font-medium bg-slate-100 px-3 py-1 rounded-full">
            {{ categories.find(c => c.value === post.content_category)?.icon || '📝' }}
            {{ post.content_category }}
          </span>
          <span v-if="post.post_type === 'shared'" class="bg-green-100 text-green-700 px-3 py-1 rounded-full font-medium">
            🔄 Shared
          </span>
        </div>
      </div>
